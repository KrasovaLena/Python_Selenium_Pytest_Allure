...
def test_task2_broken_images(browser_impl_wait):
    brokenImg = 0
...
def test_task2_checkboxes(browser_impl_wait):
    browser_impl_wait.get(task2_checkboxes)
    browser_impl_wait.find_element(*checkbox_1).click()
    assert browser_impl_wait.find_element(*checkbox_1).is_selected(), 'Чек-бокс №1 не помечен галочкой'
    browser_impl_wait.find_element(*checkbox_2).click()
    assert not browser_impl_wait.find_element(*checkbox_2).is_selected(), 'Чек-бокс №2 помечен галочкой'